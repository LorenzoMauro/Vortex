...
#include "RayData.h"
#include "Utils.h"
#include "Mdl/MdlStructs.h"
#include <optix.h>
namespace vtx
{
	extern "C" __constant__ LaunchParams optixLaunchParams;

	__forceinline__ __device__ LightSample meshLightSample(const LightData& light, PerRayData* prd)
	{
		...
		int sbtIndex = hitP.shaderConfiguration->idxCallEvaluateMaterial;
	...
	__forceinline__ __device__ LightSample envLightSample(const LightData& light, PerRayData* prd)
	{
	...
	extern "C" __device__ LightSample sampleLight(PerRayData * prd, const HitProperties * hitP)
	{
		...
		if (const int numLights = optixLaunchParams.numberOfLights; numLights > 0)
		{
			...
			switch (typeLight)
			{
				case L_MESH:
				{
					lightSample = meshLightSample(light, prd);
				}
				break;
				case L_ENV:
				{
					lightSample = envLightSample(light, prd);
				}
				break;
				default: {
